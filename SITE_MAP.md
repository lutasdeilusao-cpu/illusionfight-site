...
> Atualizado em 2026-09-25 — `SITE_VERSION` **10.292.0**.
...
- **`/historias`** (`historias/HistoriasHub.jsx`) — redesign 25/09/2026 (v10.292.0) no padrão do WEB SHARD: cabeçalho `IF // HISTÓRIAS`, destaque da linha principal (capa oficial do WEB SHARD), "continuar lendo" por título, estante numerada (`TituloCard`) com filtro do farol, **próximos capítulos** com a cascata assinante → conta → público (`CascataLiberacao`) e **capítulos recentes**. `/historias/contos` é o mesmo hub com `tipo="conto"`.
...
|---|---|---:|
| `SITE_VERSION` | Site global | **10.292.0** |
