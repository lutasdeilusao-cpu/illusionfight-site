...
*Última atualização: 2026-06-13**Versão: 10.75.0*  |  `[SITE] versão carregada: 10.75.0`  |  🌐 **`https://illusionfight.com/`** (domínio oficial)  |  ✅ Top Trumps MP v5.11.0 — cron job `limpar-salas-fantasma` reduzido de `*/5 * * * *` (5 min) para `0 3 * * *` (1x/dia às 3h da manhã). Salas fantasmas agora são limpas apenas diariamente, reduzindo carga no banco.*Versão: 10.74.0*  |  `[SITE] versão carregada: 10.74.0`  |  🌐 **`https://illusionfight.com/`** (domínio oficial)  |  ✅ Gate de assinaturas + Banner de beta — assinaturas bloqueadas antes de 14/09/2026 (admin bypass: isaiasgamedev@gmail.com vê fluxo normal); TrialBanner reativado como aviso de beta não-bloqueante com CTA para reportar bugs (oculto para admin). i18n pt/en/es. `src/config/launch.js` centraliza `LAUNCH_DATE` e `ADMIN_EMAILS`.
