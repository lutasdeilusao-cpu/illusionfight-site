...
- **Sistema de notificação (achievement toast, CTA de conta, "dicas" LDI Tip) DESLIGADO — só a Rádio Nina continua** (pedido do Isaias em 13/09/2026: achou o sistema, que ele mesmo pediu no passado, chato e invasivo demais jogando de verdade — "vira uma piscina de notificação"; temporário, ele quer repensar o conceito depois). Arquitetura: 3 produtores (`AchievementsContext.jsx` push de `achievement`/`cta_conta`, `LDINotification.jsx` push de `ldi_tip` a cada 5min, `notificationStore.js`/Tamagoshi push de `ldi_tip`) alimentam uma fila única em `localStorage` (`notificationManager.js`), lida por `UnifiedNotification.jsx` (montado globalmente em `App.jsx`). A Rádio Nina é **completamente separada**: seu convite (`RadioNina.jsx`) escreve direto em `window.__ninaPendingNotification`, nunca passa pelo `notificationManager` — só é renderizado pelo mesmo `UnifiedNotification.jsx` (branch `nina_music`, prioridade sobre a fila). Por isso desligar em UM SÓ PONTO: `notificationManager.push()` virou no-op (flag `SISTEMA_ATIVO = false` no topo do arquivo) — os 3 produtores continuam chamando `push()` sem saber que é vazio, e a Nina nunca passou por ali mesmo. Achievements continuam sendo gravados no Supabase normalmente (só o toast que some). Reativar: virar a flag pra `true` — nenhum outro arquivo precisa mudar.
