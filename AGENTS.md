...
- **Lobby multiplayer compartilhado é separado das máquinas dos jogos** — a rota isolada `/games/multiplayer/lobby` e `useSharedLobbyMachine` cuidam apenas de configuração, busca, cancelamento e formação da sala. Toda fila é identificada obrigatoriamente por `gameId:modeId:rulesVersion`; jogos ou versões diferentes nunca podem compartilhar candidatos. O teste isolado usa Supabase Realtime Presence de verdade e par ou ímpar para provar sincronização entre dois clientes. Após `MATCH_FOUND`, a máquina específica do jogo assume a partida.
