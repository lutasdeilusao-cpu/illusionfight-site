...
- **Vinheta de abertura cobre TODA carga, não só a primeira do dia** (pedido do Isaias em 12/09/2026: reportou ver texto cru piscando antes do site carregar "de vez em quando"). Causa: `#root` chega pro navegador com HTML estático de SEO (gerado por `scripts/prerender-routes.js`, texto sem estilo real, existe só pra Google/crawlers) até o bundle React montar por cima; a vinheta (`#ldi-intro` em `index.html`) existe justamente pra cobrir esse vão, mas a versão antiga fazia `el.remove()` de cara fora do primeiro acesso do dia — expondo o texto cru em toda recarga subsequente. Fix: a vinheta nunca mais é removida sem cobrir nada. No primeiro acesso do dia continua o show completo (logo animado, som, mínimo 2s/600ms reduced-motion, teto 4s). Em qualquer outra carga no mesmo dia (reload, aba nova, link externo, resultado de busca) entra em modo silencioso — mesma classe `--static` do reduced-motion (logo visível na hora, sem animação), sem som, `MIN=0` (some assim que `ldi:ready` dispara), mantendo o teto de 4s como rede de segurança. Não remover essa cobertura "silenciosa" achando que é redundante — é ela que resolve o bug.
