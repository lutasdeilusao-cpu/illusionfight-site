...
# ── Game registry ───────────────────────────────────────────────────────────
# All version constants are centralized in src/config/version.js.
# Each game maps to its constant name and SITE_MAP.md context markers.
...
GAMES = {
    ...
    "toptrumps": {
        ...
        "file": ROOT / "src/config/version.js",
        "label": "Top Trumps SP",
        "sm_context": ["Top Trumps LDI", "TS versão", "Top Trumps —"],
    },
    "toptrumps-mp": {
        "const": "TM_VERSION",
        "file": ROOT / "src/config/version.js",
        "label": "Top Trumps MP",
        "sm_context": ["TM versão", "Top Trumps MP"],
    },
    "minigames": {
    ...
    "tatics": {
        "const": "TATICS_VERSION",
        "file": ROOT / "src/config/version.js",
        "label": "Arena LDI Tatics",
        "sm_context": ["TATICS versão", "LDI Tatics"],
    },
    "site": {
        "const": "SITE_VERSION",
        "file": ROOT / "src/config/version.js",
...
def _cmd_to_str(cmd):
    """Convert a list of args to a shell command string with proper quoting."""
    def quote(s):
...
def run(cmd, cwd=None, capture=False):
    ...
    print(f"\n  -> {' '.join(cmd)}")
    # Windows: use shell=True so npm/git are found via PATHEXT (.cmd/.exe)
    use_shell = sys.platform.startswith("win")
    cmd_str = _cmd_to_str(cmd) if use_shell else cmd
    try:
...
def get_version(filepath, const_name):
    ...
    if not m:
        print(f"  [FAIL] Could not find {const_name} in {filepath}")
        sys.exit(1)
    return m.group(1)
...
def bump_version(version_str, part="patch"):
    ...
    if len(parts) != 3:
        print(f"  [FAIL] Version '{version_str}' is not semver X.Y.Z")
        sys.exit(1)
...
def replace_version_in_file(filepath, old_ver, new_ver, const_name):
    ...
    if new_content == content:
        print(f"  [FAIL] Failed to replace {old_ver} in {filepath}")
        sys.exit(1)
    filepath.write_text(new_content, encoding="utf-8")
    print(f"  [OK] {filepath.relative_to(ROOT)}: {old_ver} -> {new_ver}")
...
def update_site_map(game_key, old_ver, new_ver):
    ...
    if updated:
        ...
        print(f"  [OK] SITE_MAP.md: {updated} occurrence(s) of {old_ver} -> {new_ver}")
    else:
        print(f"  [WARN] SITE_MAP.md: no lines matched for {game_key} with version {old_ver}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which game(s) were modified (ldi, jack, toptrumps, toptrumps-mp, minigames, arena, pp, tatics, site)",
    )
    parser.add_argument(
        "-m", "--message",
        required=True,
