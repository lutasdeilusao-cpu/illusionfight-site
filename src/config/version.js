 ...
 */
 ...
 // ── Site ──────────────────────────────────────────
export const SITE_VERSION = '10.280.189' // fix: LDI Gangues - modal de ficha do recrutamento (corpo inteiro) nao precisa mais de scroll interno; espaco vazio acima do card (antes so em cima) agora se divide em cima e embaixo. Ver GANGUES_VERSION.

// ── Games ─────────────────────────────────────────
export const PP_VERSION        = '2.3.1'  // Pesadelo Particular — fix: guest i18n keys movidas para o namespace pp em pt/en/es.json
export const LDI_VERSION       = '2.0.1'  // Lendas do LDI — guest aviso melhorado no lobby (título, texto explicativo, link cadastro)
export const JACK_VERSION      = '5.3.2'  // Jack Dream Beer — correção de encoding em comentário
export const GANGUES_VERSION   = '3.25.0' // fix (Isaias, print do modal de recrutamento no DevTools mobile: "sobra espaco la em cima, o que tira totalmente a necessidade desse scroll, a ficha deve caber na tela e basta utilizar o espaco melhor"): hero de corpo inteiro (`.gang-sheet-modal__corpo-wrap`/`-img`) reduzido de 300/320px pra 230/250px e stats/PV-PM/tecnica/botao com padding/gap mais justos - o conteudo da ficha (com a arte de corpo, §15.1 do GDD) passava do `max-height` do card e forcava scroll interno mesmo sem nada de exotico na ficha. `.gang-sheet-modal` trocou `align-items: end` por `center` - a folga que sobra agora se divide em cima E embaixo, em vez de empilhar tudo no topo. Verificado ao vivo com Playwright no viewport exato do print (393x852): `scrollHeight === clientHeight` (zero scroll necessario), gap simetrico (75px em cima, 75px embaixo). Trilha anterior (3.24.0): animacao de combate (ataqueNormal+dano) pra Fenda/Catraca/Faisca, completando os 5 personagens iniciais, + som de impacto por genero (soco-leve/dano-leve novos pras mulheres, sons do Trinca/Muro reaproveitados pro Faisca).
