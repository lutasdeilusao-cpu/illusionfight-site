 ...
 */
 ...
 // ── Site ──────────────────────────────────────────
export const SITE_VERSION = '10.280.112' // fix: pagina privada /preview-privado/webtoon-00-comparar quebrada (imagens sumiram) - causa raiz: as 2 imagens (webtoon-00-novo.png ~90MB, webtoon-00-antigo.png ~28MB) nunca fizeram parte do build (grandes demais pro git principal), foram coladas manualmente em dist/ na 1a vez; qualquer `npm run deploy` seguinte roda o hook `predeploy` (`npm run build`), o Vite limpa dist/ inteiro, e as imagens somem silenciosamente - foi exatamente o que aconteceu nos deploys de rotina de 12-13/09/2026. Recuperadas do proprio historico do branch gh-pages (nunca reescrito, so ganha commit novo por deploy) e agora restauradas automaticamente: `scripts/restaurar-preview-privado.cjs` reextrai os 2 blobs de um commit fixo do gh-pages e copia pra dist/preview-privado/, chamado no fim do `predeploy` em package.json - todo deploy futuro se auto-recupera, ninguem precisa lembrar de copiar manualmente de novo.
