 ...
 */
 ...
 // ── Site ──────────────────────────────────────────
export const SITE_VERSION = '10.281.1' // fix(deploy): `npm run deploy` trocado de `gh-pages -d dist` pra `node scripts/deploy-gh-pages.cjs` — a lib gh-pages limpa o branch antigo passando CADA arquivo rastreado como argumento separado pro `git rm` (~1400 hoje), o que estourou o limite de linha de comando do Windows (ENAMETOOLONG) em 22/09/2026 e foi contornado na hora com --add (que so ADICIONA, nunca remove — o branch ia inchar pra sempre). O script novo faz a mesma limpeza com `git rm -r -f -- .` (1 argumento so, git resolve a recursao por dentro, sem limite de SO) num git worktree local do gh-pages; testado de ponta a ponta contra um branch descartavel antes de trocar o script real (push aceito, arvore final batendo 1:1 com dist/, sem sobra de arquivo antigo, historico do gh-pages preservado sem force-push).
