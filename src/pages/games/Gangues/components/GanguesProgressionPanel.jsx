...
export default function GanguesProgressionPanel({ member, onApply, onClose, onDelete }) {
  const { t } = useLanguage()
  ...
  return <section className={`gang-progression gang-progression--${member.combat_path}`}>
    <div className="gang-progression-header">
      ...
      <div className="gang-progression-headline"><h2>{t('games.gangues.progression.title')}</h2><p>{member.sheet_name} · {t(`games.gangues.loadout.paths.${member.combat_path}.name`)}</p></div>
      <div className="gang-progression-header-actions">
        ...
        {onClose && <button className="gang-progression-close" onClick={onClose} aria-label={t('games.gangues.progression.close')}>×</button>}
      </div>
    </div>
    ...
    <div className="gang-attr-upgrade-grid">{ATTRIBUTES.map(attribute => {
      const value = Number(member.attributes?.[attribute]) || 0
      const cost = GANGUES_ATTRIBUTE_XP_COSTS[value]
      return <button key={attribute} className="gang-attr-upgrade-card" disabled={!cost || progression.xp_unspent < cost} onClick={() => onApply(member, upgradeGanguesAttribute(member, attribute))}><span className="gang-attr-upgrade-letter">{attribute}</span><span className="gang-attr-upgrade-values">{value} <b>→</b> {value + 1}</span><span className="gang-attr-upgrade-cost">{cost ? t('games.gangues.progression.upgrade', { n: cost }) : '—'}</span></button>
    })}</div>
    ...
    {!unlockedPath && <button className="gang-special-path-unlock" disabled={!canUnlockPath} onClick={() => onApply(member, selectGanguesSpecialPath(member, previewPath?.id))}>{pathRequirementMet ? t('games.gangues.progression.unlock_path', { name: t(`games.gangues.progression.paths.${previewPath?.id}`), n: GANGUES_SPECIAL_PATH_XP_COST }) : t('games.gangues.progression.path_requirement', { n: attributeTotal, required: GANGUES_SPECIAL_PATH_ATTRIBUTE_REQUIREMENT })}</button>}
    ...
    <div className="gang-skill-grid">{specials.map(special => {
      ...
      return <div key={special.id} className={`gang-skill-node ${equipped ? 'gang-skill-node--equipped' : ''} ${level === 0 ? 'gang-skill-node--locked' : ''}`}><div className="gang-skill-node-icon">{SPECIAL_ICON[previewPath?.id] || '★'}</div><strong className="gang-skill-node-name">{t(`games.gangues.progression.skills.${special.id}`)}</strong><span className="gang-skill-node-kind">{t(`games.gangues.progression.${special.kind}`)}</span><div className="gang-skill-node-pips">{[0, 1, 2].map(pip => <i key={pip} className={pip < level ? 'gang-skill-pip gang-skill-pip--filled' : 'gang-skill-pip'} />)}</div><div className="gang-skill-node-actions"><button disabled={!purchasable || !cost || progression.xp_unspent < cost} onClick={() => onApply(member, upgradeGanguesSpecial(member, special.id))}>{purchasable && cost ? t('games.gangues.progression.upgrade', { n: cost }) : t('games.gangues.progression.unlock_path_first')}</button>{purchasable && level > 0 && <button className="gang-skill-node-toggle" disabled={!equipped && progression.selected_specials.length >= 2} onClick={() => onApply(member, toggleGanguesSpecial(member, special.id))}>{t(`games.gangues.progression.${equipped ? 'unequip' : 'equip'}`)}</button>}</div></div>
    })}</div>
  </section>
}
