...
/* CLUBE DA LUTA — tela de desfecho (fim do gauntlet OU derrota em qualquer
   ronda). É a cena mais pesada do modo história: holofote, plateia em
   silhueta, grão de filme, faíscas / caco de caderneta na vitória, vinheta
   pulsando e "zumbido de ouvido" na derrota. Tudo CSS/framer + sfx sintetizado
...
export default function GanguesClubeResultado({ victory, entrouLimpo, t, onVoltar }) {
  ...
  const titulo = t(victory ? 'games.gangues.clube.venceu_titulo' : 'games.gangues.clube.perdeu_titulo')
  const texto = t(victory
    ? (entrouLimpo ? 'games.gangues.clube.venceu_texto_grana' : 'games.gangues.clube.venceu_texto')
    : 'games.gangues.clube.perdeu_texto')

  return (
    <main className={`gang-clube-cena gang-clube-cena--${victory ? 'vitoria' : 'derrota'}${R ? ' is-reduced' : ''}`}>
      ...
      <div className="gang-clube-cena-conteudo">
        <motion.span
          className="gang-clube-cena-code"
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
          {t('games.gangues.clube.result_code')}
        </motion.span>

        <motion.h1
          className="gang-clube-cena-titulo"
