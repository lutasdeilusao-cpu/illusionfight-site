    ...
    Object.entries(distrito.exits).forEach(([dir, exitData]) => {
      ...
      for (let ty = startTy; ty <= endTy; ty++) {
        for (let tx = startTx; tx <= endTx; tx++) {
          ...
          ctx.fillRect(ex + 2, ey + 2, STEP - 4, STEP - 4)
    ...
    function checkZones() {
      ...
      if (exit) {
        const exitName = exit.to === 'central' ? t('tatics.zones.central')
          : exit.to === 'residencial' ? t('tatics.zones.residencial')
          : exit.to === 'comercial' ? t('tatics.zones.comercial')
          : exit.to === 'industrial' ? t('tatics.zones.industrial')
          : exit.to === 'porto' ? t('tatics.zones.porto')
