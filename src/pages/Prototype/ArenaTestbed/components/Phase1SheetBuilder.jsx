...
export default function Phase1SheetBuilder({ onConfirm }) {
  ...
  function updateChar(idx, field, value) {
    ...
    // Se mudar tipo de ataque, zera o atributo bloqueado
    if (field === 'tipoAtaque') {
      if (value === 'melee') {
        updated[idx].pdf = 0
      } else {
  ...
  const canProceed = useMemo(() => {
    ...
    return hasPlayer && hasIA && allNamed && characters.length > 0
  }, [characters])
          ...
          <div key={char.id} className={`tab-fase1-card ${editing === idx ? 'editing' : ''}`}>
            ...
            <div className="tab-fase1-radio-group">
              ...
              <label className="tab-fase1-radio">
                <input
                  type="radio"
                  name={`atk_${idx}`}
                  checked={char.tipoAtaque === 'distancia'}
            ...
            <div className="tab-fase1-attrs">
              {ATTRIBUTES.map(attr => {
                ...
                const val = isBlocked ? 0 : (char[attr] || 1)
                return (
                  <div key={attr} className="tab-fase1-attr-row">
                    ...
                    <div className="tab-fase1-attr-controls">
                      <button
                        ...
                        disabled={val <= 1 || isBlocked}
                        onClick={() => updateAttr(idx, attr, -1)}
                      >−</button>
                      <span className={`tab-fase1-attr-val ${isBlocked ? 'blocked' : ''}`}>
                        {val}
                      </span>
                      <button
                        className="tab-btn-small"
            ...
            <div className="tab-fase1-items">
              ...
              <label className="tab-fase1-item-row">
                ...
                <div className="tab-fase1-attr-controls">
                  <button className="tab-btn-small" disabled={char.pocaoHP <= 0} onClick={() => updateChar(idx, 'pocaoHP', Math.max(0, char.pocaoHP - 1))}>−</button>
                  <span className="tab-fase1-attr-val">{char.pocaoHP}</span>
                  <button className="tab-btn-small" disabled={char.pocaoHP >= 5} onClick={() => updateChar(idx, 'pocaoHP', char.pocaoHP + 1)}>+</button>
                </div>
              </label>
              <label className="tab-fase1-item-row">
                <span>{t('prototype.arena_testbed.potion_mp')} (+5)</span>
                <div className="tab-fase1-attr-controls">
                  <button className="tab-btn-small" disabled={char.pocaoMP <= 0} onClick={() => updateChar(idx, 'pocaoMP', Math.max(0, char.pocaoMP - 1))}>−</button>
                  <span className="tab-fase1-attr-val">{char.pocaoMP}</span>
                  <button className="tab-btn-small" disabled={char.pocaoMP >= 5} onClick={() => updateChar(idx, 'pocaoMP', char.pocaoMP + 1)}>+</button>
                </div>
              </label>
            </div>
