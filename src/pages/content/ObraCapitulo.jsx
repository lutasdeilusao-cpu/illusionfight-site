...
export default function ObraCapitulo() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const path = `../../data/livro/obras/${slug}/${lang}/${cap}.md`
      const loader = obraLoaders[path] || obraLoaders[`../../data/livro/obras/${slug}/pt/${cap}.md`]
      if (loader) {
    ...
    return (
      <section className="livro-capitulo">
        ...
        <div className="container">
          ...
          <button className="livro-capitulo__back" onClick={() => navigate(`/historias/${slug || ''}`)}>
            {t('pages.obra.voltar_livro')}
          </button>
        </div>
      </section>
    )
  ...
  return (
    <section className="livro-capitulo">
      <Helmet>
        <title>{capitulo ? `${capitulo[tituloKey]} — ${obra[tituloKey]}` : t('pages.helmet.capitulo_nao_encontrado')}</title>
      </Helmet>
      <div className="container">
        <nav className="livro-linhas livro-linhas--reader">
          <Link to="/historias" className="livro-linha">{t('pages.historias.titulo')}</Link>
          <Link to={`/historias/${slug}`} className="livro-linha">{obra[tituloKey]}</Link>
        </nav>
        ...
        <div className="livro-capitulo__nav">
          {anterior ? (
            <button className="livro-capitulo__nav-btn" onClick={() => navigate(`/historias/${slug}/${anterior.id}`)}>
              ← {anterior[tituloKey]}
            </button>
          ) : (
          ...
          {proximo ? (
            <button className="livro-capitulo__nav-btn" onClick={() => navigate(`/historias/${slug}/${proximo.id}`)}>
              {proximo[tituloKey]} →
            </button>
          ) : (
            <span className="livro-capitulo__nav-btn livro-capitulo__nav-btn--hidden">→</span>
