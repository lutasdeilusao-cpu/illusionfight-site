  ...
  return (
    <div className="modal-lancamento-overlay" onClick={handleFechar}>
      <div className="modal-lancamento" onClick={e => e.stopPropagation()}>
        <h2 className="modal-lancamento-titulo">{t('site.modal_lancamento.titulo')}</h2>
        <p className="modal-lancamento-corpo">{t('site.modal_lancamento.corpo')}</p>
        {!user ? (
          <>
            <p className="modal-lancamento-cta-texto">{t('site.modal_lancamento.cta_guest')}</p>
            <button className="modal-lancamento-btn" onClick={() => { handleFechar(); navigate('/cadastro') }}>
              {t('site.modal_lancamento.botao_conta')}
            </button>
          </>
        ) : (
          <p className="modal-lancamento-logado">{t('site.modal_lancamento.logado')}</p>
        )}
        <button className="modal-lancamento-fechar" onClick={handleFechar}>
          {t('site.modal_lancamento.fechar')}
        </button>
      </div>
    </div>
  )
