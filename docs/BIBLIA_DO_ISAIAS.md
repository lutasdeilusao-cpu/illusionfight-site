...
>
> **#005 — 2026-09-16:** Agente rodou `npm run build` para validar a máquina de animação de combate (Gangues), depois editou `src/config/version.js` (bump de versão, passo obrigatório do workflow) SEM rebuildar antes de commitar/deployar — o `dist/` publicado era de antes do bump. O agente comparou hash de ARQUIVO (`index-CmM_Vt5U.js`) entre local/gh-pages/site ao vivo, os três bateram, e relatou "confirmado no ar" — mas o conteúdo do bundle continuava com a versão antiga (3.15.0), porque o hash é do build velho, não prova nada sobre o CONTEÚDO. Isaias abriu o DevTools ao vivo e viu a versão errada no console. **Lição:** a ordem correta é SEMPRE bump de versão → build → commit → push → deploy, nessa sequência, nunca build antes do bump. **Verificação de deploy nunca é só comparar nome de hash** — tem que baixar/grepar o JS publicado e confirmar que a STRING DA VERSÃO NOVA está literalmente dentro do arquivo (`grep 'X.Y.Z' dist/assets/index-*.js`), antes de qualquer relato de sucesso.
